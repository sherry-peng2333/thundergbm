...
//template<typename T>
//__global__ void reduce_kernel(T *&input) {
...
void HistTreeBuilder::find_split_mo(int level, int device_id) {
    std::chrono::high_resolution_clock timer;
    ...
    //find the best split locally
    {
        ...
        //calculate split information for each split
        SyncArray<GHPair> hist(n_split*d_outputs_);
        SyncArray<GHPair> missing_gh(n_partition*d_outputs_);
        SyncArray<float_type> hist_per_split_g(n_instances*d_outputs_);
        SyncArray<float_type> hist_per_split_h(n_instances*d_outputs_);
        auto cut_fid_data = cut.cut_fid.device_data();
        auto i2fid = [=] __device__(int i) { return cut_fid_data[i % n_bins]; };
        auto hist_fid = make_transform_iterator(counting_iterator<int>(0), i2fid);
        {
...
//                                                    atomicAdd(&dest.g, src.g);
                                    ...
                                    int num_fv = (idx_end - idx_begin) * n_column;
                                    for(int did = 0; did < d_outputs_ ; did++) {
                                        hist_data = hist.device_data() + did * n_split + nid0 * n_bins;
                                        anonymous_kernel([=] __device__() {
                                            extern __shared__ GHPair local_hist[];
                                            for (int i = threadIdx.x; i < n_bins; i += blockDim.x) {
                                                local_hist[i] = 0;
                            ...
                            //subtract
                            int n_split_last = static_cast<int>(pow(2, level-1)) * n_bins;
                            auto t_copy_start = timer.now();
                            {
                                auto hist_data_computed = hist.device_data() + nid0_to_compute * n_bins;
                                auto hist_data_to_compute = hist.device_data() + nid0_to_substract * n_bins;
                                ...
                                device_loop(n_bins * d_outputs_, [=]__device__(int i) {
                                    int d = i / n_bins;
                                    int bid = i % n_bins;
                                    hist_data_to_compute[d*n_split+bid] = father_hist_data[d*n_split_last+bid]-hist_data_computed[d*n_split+bid];
                                });
                            }
                            auto t_copy_end = timer.now();
                            std::chrono::duration<double> cp_used_time = t_copy_end - t_copy_start;
                ...
                device_loop(n_partition, [=]__device__(int pid) {
                    ...
                    if (cut_row_ptr[fid + 1] != cut_row_ptr[fid]) {
                        for(int i = 0; i < d_outputs_; i++){
                            GHPair node_gh = hist_data[i*n_split+nid0 * n_bins+cut_row_ptr[fid + 1] - 1];
                            missing_gh_data[i*n_partition+pid] = sum_gh_pair_mo_data[nid*d_outputs_+i] - node_gh;
                        }
                    }
                });
                LOG(DEBUG) << missing_gh;
        ...
        {
//            TIMED_SCOPE(timerObj, "calculate gain");
            auto compute_gain = []__device__(GHPair father, GHPair lch, GHPair rch, float_type min_child_weight, float_type lambda) -> float {
                if (lch.h >= min_child_weight && rch.h >= min_child_weight)
                    return (lch.g * lch.g) / (lch.h + lambda) + (rch.g * rch.g) / (rch.h + lambda) -
                           (father.g * father.g) / (father.h + lambda);
                else
...
//                    auto father_gh = nodes_data[nid].sum_gh_pair.host_data();
                    ...
                    for(int j = 0; j < d_outputs_; j++){
                        GHPair father_gh = sum_gh_pair_mo_data[nid*d_outputs_+j];
                        GHPair rch_gh = gh_prefix_sum_data[j*n_split+i];
                        default_to_left_gain = default_to_left_gain +
                                               compute_gain(father_gh, father_gh - rch_gh, rch_gh, mcw, l);
                    }
                    default_to_left_gain = max(0.f, default_to_left_gain);
                    for(int j = 0; j < d_outputs_; j++){
                        GHPair father_gh = sum_gh_pair_mo_data[nid*d_outputs_+j];
                        GHPair rch_gh = gh_prefix_sum_data[j*n_split+i]+missing_gh_data[j*n_partition+pid];
                        default_to_right_gain = default_to_right_gain+
                                                compute_gain(father_gh, father_gh - rch_gh, rch_gh, mcw, l);
                    }
//                    printf("default_to_right_gain: %f\n", default_to_right_gain/4);
                    default_to_right_gain = max(0.f, default_to_right_gain);
                    if (default_to_left_gain > default_to_right_gain)
                        gain_data[i] = 0.5/d_outputs_*default_to_left_gain;
                    else
                        gain_data[i] = -0.5/d_outputs_*default_to_right_gain;//negative means default split to right
...
//            LOG(INFO) << "best rank & gain = " << best_idx_gain;
        ...
        //get split points
        {
            ...
            sp.resize(n_nodes_in_level);
            sp_fea_missing_gh.resize(n_nodes_in_level * d_outputs_);
            sp_rch_sum_gh.resize(n_nodes_in_level * d_outputs_);
            auto sp_data = sp.device_data();
            auto nodes_data = tree.nodes.device_data();
            int column_offset = columns.column_offset;
            auto cut_row_ptr_data = cut.cut_row_ptr.device_data();
            ...
            device_loop(n_nodes_in_level, [=]__device__(int i) {
                ...
                sp_data[i].default_right = best_split_gain < 0;
                for(int j = 0; j < d_outputs_; j++){
                    sp_fea_missing_gh_data[i * d_outputs_ +j] = missing_gh_data[j * n_partition + i * n_column + hist_fid[split_index]];
                    sp_rch_sum_gh_data[i * d_outputs_ +j] = hist_data[j * n_split + split_index];
                }
            });

        }
    }
//    LOG(INFO) << "split points (gain/fea_id/nid): " << sp;
    LOG(DEBUG) << "split points (gain/fea_id/nid): " << sp;
}
...
void HistTreeBuilder::init(const DataSet &dataset, const GBMParam &param) {
    ...
    DO_ON_MULTI_DEVICES(param.n_device, [&](int device_id){
        ...
        last_hist[device_id].resize((2 << param.depth) * cut[device_id].cut_points_val.size()*d_outputs_);
    });
    get_bin_ids();
    for (int i = 0; i < param.n_device; ++i) {
        v_columns[i].release();
