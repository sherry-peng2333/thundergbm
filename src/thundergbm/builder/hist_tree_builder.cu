                            ...
                            device_loop(n_instances * n_column, [=]__device__(int i) {
                                ...
                                if (bid != max_num_bin) {
                                    ...
                                    for(int j = 0; j < d_outputs_; j++){
                                        ...
                                        GHPair &dest = hist_data[j*n_max_splits+feature_offset + bid];
                                        if(src.h != 0)
                                            atomicAdd(&dest.h, src.h);
                                        if(src.g != 0)
                                            atomicAdd(&dest.g, src.g);
...
//                                        printf("i: %d  iid: %d fid: %d", i, iid, fid);
                            ...
                            auto t_copy_start = timer.now();
                            {
                                ...
                                device_loop(n_bins * d_outputs_, [=]__device__(int i) {
                                    int d = i / n_bins;
                                    int bid = i % n_bins;
                                    hist_data_to_compute[d*n_max_splits+bid] = father_hist_data[d*n_max_splits+bid]-hist_data_computed[d*n_max_splits+bid];
                                });
                            }
                            auto t_copy_end = timer.now();
                            std::chrono::duration<double> cp_used_time = t_copy_end - t_copy_start;
                ...
                device_loop(n_partition, [=]__device__(int pid) {
                    ...
                    if (cut_row_ptr[fid + 1] != cut_row_ptr[fid]) {
                        for(int i = 0; i < d_outputs_; i++){
                            GHPair node_gh = hist_data[i*n_max_splits+nid0 * n_bins+cut_row_ptr[fid + 1] - 1];
        ...
        //get split points
        {
            ...
            device_loop(n_nodes_in_level, [=]__device__(int i) {
                ...
                for(int j = 0; j < d_outputs_; j++){
                    ...
                    sp_rch_sum_gh_data[i * d_outputs_ +j] = hist_data[j * n_max_splits + split_index];
                }
            });
