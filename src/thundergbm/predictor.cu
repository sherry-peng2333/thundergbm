...
void Predictor::predict_raw(const GBMParam &model_param, const vector<vector<Tree>> &boosted_model,
    ...
    int d_outputs = model_param.d_outputs;
    //TODO: reduce the output size for binary classification
    y_predict.resize(n_instances * num_class * d_outputs);

    SyncArray<Tree::TreeNode> model(total_num_node);
    SyncArray<float_type> weight(total_num_node * d_outputs);
    auto model_data = model.host_data();
    auto weight_data = weight.host_data();
    int tree_cnt = 0;
    for (auto &vtree:boosted_model) {
        for (auto &t:vtree) {
            memcpy(model_data + num_node * tree_cnt, t.nodes.host_data(), sizeof(Tree::TreeNode) * num_node);
            for (int i = 0; i < num_node; i++){
                const Tree::TreeNode &src_node = t.nodes.host_data()[i];
                if(src_node.base_weight.size())
                    memcpy(weight_data + num_node * tree_cnt * d_outputs + i * d_outputs,
                           src_node.base_weight.host_data(), sizeof(float_type) * d_outputs);
            }
            tree_cnt++;
        }
    }
    ...
    //do prediction
    SyncArray<float_type> sum(n_instances * d_outputs);
    ...
    auto weight_device_data = weight.device_data();
    auto sum_data = sum.device_data();
    ...
    if (smem_size <= 1) {//48KB shared memory for P100
//        // 48 * 1024L
...
//        },n_instances * n_features, smem_size, NUM_BLOCK, BLOCK_SIZE);
        ...
        device_loop(n_instances, [=]__device__(int iid) {
            ...
            int row_len = csr_row_ptr_data[iid + 1] - csr_row_ptr_data[iid];
            for (int t = 0; t < num_class; t++) {
                auto predict_data_class = predict_data + t * n_instances * d_outputs;
                float_type *cur_sum_data = sum_data + iid * d_outputs;
                for(int sum_id = 0; sum_id < d_outputs; sum_id++){
                    cur_sum_data[sum_id] = 0;
                }
                for (int iter = 0; iter < num_iter; iter++) {
                    const Tree::TreeNode *node_data = model_device_data + iter * num_class * num_node + t * num_node;
                    //Tree::TreeNode curNode = node_data[0];
                    int cur_nid = 0; //node id
                    while (!node_data[cur_nid].is_leaf) {
                        int fid = node_data[cur_nid].split_feature_id;
                        bool is_missing;
                        float_type fval = get_val(col_idx, row_val, row_len, fid, &is_missing);
                        if (!is_missing)
                            if(fval < node_data[cur_nid].split_value)
                                cur_nid = node_data[cur_nid].lch_index;
                            else cur_nid = node_data[cur_nid].rch_index;
                            //cur_nid = get_next_child(node_data[cur_nid], fval);
                        else if (node_data[cur_nid].default_right)
                            cur_nid = node_data[cur_nid].rch_index;
                        else
                            cur_nid = node_data[cur_nid].lch_index;
                        //curNode = node_data[cur_nid];
                    }
                    int cur_nid_in_weight = iter * num_class * num_node + t * num_node + cur_nid;  //current node id in weight
                    for(int sum_id = 0; sum_id < d_outputs; sum_id++){
                        cur_sum_data[sum_id] += lr * weight_device_data[cur_nid_in_weight * d_outputs + sum_id];
                    }
//                    sum += lr * node_data[cur_nid].base_weight;
                }
                for(int sum_id = 0; sum_id < d_outputs; sum_id++){
                    predict_data_class[iid * d_outputs + sum_id] += cur_sum_data[sum_id];
                }
//                predict_data_class[iid] += sum;
